...
*/
std::string MDString(char *string)
{
	MD5_CTX context;
	std::string strMD5(64, '\0');
	unsigned char ucMD5Data[16] = { 0 };
	unsigned int len = strlen(string);

	MD5Init(&context);
	MD5Update(&context, (unsigned char *)string, len);
	MD5Final(ucMD5Data, &context);

	for (int i = 0; i < sizeof(ucMD5Data); i++)
	{
		sprintf((char *)strMD5.c_str() + i * 2, "%02X", ucMD5Data[i]);
	}
