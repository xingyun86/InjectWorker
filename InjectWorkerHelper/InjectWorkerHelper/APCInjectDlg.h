...
class CAPCInjectDlg :
	...
	public CDialogImpl<CAPCInjectDlg>,
	public CDialogLayout<CAPCInjectDlg>,
	public CUpdateUI<CAPCInjectDlg>,
	public CMessageFilter,
	public CIdleHandler
{
public:
	enum { IDD = IDD_DIALOG_APCINJECT };
	...
	BEGIN_UPDATE_UI_MAP(CAPCInjectDlg)
	END_UPDATE_UI_MAP()

	BEGIN_MSG_MAP(CAPCInjectDlg)
		...
		CHAIN_MSG_MAP(CDialogLayout<CAPCInjectDlg>)
	...
	LRESULT OnSuspendProcessInject(UINT, int, HWND)
	{
		...
		{
			//将DLL路径写入目标进程空间
			if (WriteProcessMemory(pi.hProcess, lpDllName, CStringA(strAppDll), CStringA(strAppDll).GetLength(), NULL))
			{
				LPVOID nLoadLibrary = (LPVOID)GetProcAddress(GetModuleHandle(_T("kernel32.dll")), "LoadLibraryA");
				//向远程APC队列插入LoadLibraryA
				if (!QueueUserAPC((PAPCFUNC)nLoadLibrary, pi.hThread, (DWORD)lpDllName))
				{
					MessageBox(_T("QueueUserAPC失败！！"));
					return 0;
				}
			}
			else
			{
				MessageBox(_T("WriteProcessMemory失败！！"));
				return 0;
			}
		}
